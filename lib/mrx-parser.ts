...
const MRX_LINE_LENGTH = 921;

export function parseMrxFile(content: string): ParseResult {
    ...
    rawLines.forEach((raw, index) => {
        ...
        if (firstChar === 'H') {
            type = 'Header';
            schemaFields = MrxSchema.header;
        } else if (firstChar === 'D') {
            type = 'Data';
            schemaFields = MrxSchema.data;
        } else if (firstChar === 'T') {
            type = 'Trailer';
            schemaFields = MrxSchema.trailer;
        }
        ...
        if (raw.length !== MRX_LINE_LENGTH && raw.length > 0) {
            lineIsValid = false;
            globalError = `Length Mismatch (${raw.length}/${MRX_LINE_LENGTH})`;

            if (raw.length > MRX_LINE_LENGTH) {
                alignmentTips.push(`Line is OVERFLOWING. Delete ${raw.length - MRX_LINE_LENGTH} char(s).`);
            } else {
                alignmentTips.push(`Line is SHORT. Add ${MRX_LINE_LENGTH - raw.length} space(s).`);
            }
        }
    ...
    // For MRX, we don't have Accepted/Rejected status in the same way, so count all basic valid data lines
    const dataCount = parsedLines.filter(l => l.type === 'Data').length;
    ...
    return {
        ...
        summary: {
            ...
            accepted: dataCount, // Placeholder for MRX
            rejected: 0
        }
    };
}
